...
function Students() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      email: Yup.string().email("Email xato").required("Emailni kiriting"),
      jshshr: Yup.string().length(16, "16 ta raqam bo'lsin !").required("Jshir raqamini kiriting"),
      dob: Yup.string().required("Tugulgan kuningiz"),
      address: Yup.string().required("Yashash manzil"),
    }),
    onSubmit: async (values, { resetForm }) => {
      try {
        if (editId) {
          const res = await User.updateUser(editId, values);
          if (res?.status === 200) {
            await fetchStudents();
            resetForm();
            setEditingId(null);
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="col-span-1 bg-white p-6 rounded-lg shadow-md">
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-3">
            {["first_name", "last_name", "email", "jshshr", "dob", "address"].map(
              (field) => (
                <div key={field}>
                  <input
                    type="text"
